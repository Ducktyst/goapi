...
func (s *Server) initHandlers() {
	...
	exchangeGroup := group.Group("/exchange") // создавть курс обмена. Создание записи в таблице валют
	exchangeGroup.GET("/cources", s.GetClasses)
	exchangeGroup.POST("/create", func(c *gin.Context) {
	...
	classGroup.POST("/convert", func(c *gin.Context) {
		var input database.Class
		c.ShouldBindJSON(&input)
		if err := s.db.CreateClass(c.Request.Context(), input); err != nil {
			c.Error(err)
	...
	exchangeGroup.GET("/news", s.GetClasses)
	
	// Создать вопрос
	// Ответить на вопрос (для тех. поддержки)
